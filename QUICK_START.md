...
### 2a. Configure NVIDIA Container Toolkit (First Time Only, if needed)

If the preflight check warns about NVIDIA runtime not being configured:

```bash
sudo ./scripts/init_vyasa.sh --configure-nvidia
```

This configures Docker to use NVIDIA GPUs for GPU services (cortex-brain, cortex-worker, cortex-vision).
...
- **Port conflicts**: The script automatically detects port conflicts (e.g., port 30000 for Brain). If detected, you'll see resolution options
