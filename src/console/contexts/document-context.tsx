...
export function DocumentProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!isInitialized) {
      try {
        const savedDocuments = localStorage.getItem('project_vyasa_documents')
        if (savedDocuments) {
    ...
    if (isInitialized) {
      try {
        if (documents.length > 0) {
          ...
          localStorage.setItem('project_vyasa_documents', JSON.stringify(documentsToSave));
          console.log(`Saved ${documents.length} documents to localStorage`);
        } else {
          // Clear localStorage if documents array is empty
          localStorage.removeItem('project_vyasa_documents');
  ...
  const readFileContent = (file: File): Promise<string> => {
    ...
    if (isPdf) {
      // Send PDF to backend for conversion to Markdown
      ...
      return fetch("/api/upload", {
        ...
        .then(async (response) => {
          ...
          if (!data.markdown || typeof data.markdown !== "string") {
            throw new Error("PDF conversion returned no markdown")
          }
