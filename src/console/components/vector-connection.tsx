...
interface VectorConnectionProps {
  className?: string
}

export function VectorConnection({ className }: VectorConnectionProps) {
  ...
  const fetchStats = async () => {
    try {
      ...
      setConnectionStatus("disconnected");
      const message = error instanceof ApiError ? `${error.status}: ${error.message}` : error instanceof Error ? error.message : 'Error connecting to vector database'
      setError(message);
    }
  };
  ...
  return (
    <div className={`flex flex-col items-start space-y-4 p-4 border rounded-md ${className}`}>
      <div className="flex justify-between w-full">
        <h2 className="text-lg font-medium">Vector DB Status</h2>
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger>
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    const message = err instanceof ApiError ? `${err.status}: ${err.message}` : err instanceof Error ? err.message : 'Error creating collection'
                    setError(message);
                    setConnectionStatus("disconnected");
                  }
                }}
      ...
      <div className="text-sm space-y-1 w-full">
        <div className="flex justify-between">
          ...
          <span className="text-xs text-muted-foreground">{(stats as any).url || 'http://vyasa-qdrant:6333'}</span>
        </div>
        <div className="flex justify-between">
