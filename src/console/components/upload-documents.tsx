...
export function UploadDocuments() {
  ...
  const validateFiles = (files: File[]): File[] => {
    ...
    const validFiles = Array.from(files).filter((file) => {
      const isValidType = file.name.endsWith(".md") || file.name.endsWith(".csv") || file.name.endsWith(".txt") || file.name.endsWith(".json") || file.name.endsWith(".pdf")
      if (!isValidType) {
        setError("Only markdown (.md), CSV (.csv), text (.txt), JSON (.json), and PDF (.pdf) files are supported.")
        return false
      }
      return true
    })
      ...
      >
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="inline-flex items-center gap-2 text-xs text-muted-foreground bg-muted/40 px-3 py-1.5 rounded-full border border-border/30">
            ...
            <span>.md, .csv, .txt, .json, .pdf supported</span>
          </div>
        </div>
      </div>
    </div>
